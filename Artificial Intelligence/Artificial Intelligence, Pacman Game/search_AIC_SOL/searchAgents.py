...
def cornersHeuristic(state, problem):
    ...
    if problem.isGoalState(state):
     ...
     for c in corners:
       if c not in state[1]:
         costheuri=costheuri+1
         notvisitedcorners =notvisitedcorners + [c]
     xy1=state[0]
     while len(notvisitedcorners) != 0:
       m=9999999
       for c in notvisitedcorners:
            if util.manhattanDistance( xy1, c ) < m:
               m=util.manhattanDistance( xy1, c )
               i=c
       costheuri=costheuri+m
       xy1=i
       notvisitedcorners.remove(i)
      
     return costheuri# Default to trivial solution
